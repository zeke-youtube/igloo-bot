...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((option) => option.setName('amount').setDescription('Number of messages to delete, from 1 to 10000. Leave empty to clear the channel.').setRequired(false).setMinValue(1).setMaxValue(10000)),
  async execute(interaction) {
    ...
    try {
      ...
      let previousDeleted = -1;
      while (deletedTotal < target && deletedTotal !== previousDeleted) {
        previousDeleted = deletedTotal;
        const batchSize = amount ? Math.min(100, amount - deletedTotal) : 100;
