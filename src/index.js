...
for (const file of ['about', 'help', 'pengfact', 'pikastudiosites', 'announcements', 'coinflip', 'clear', 'global-cooldown', 'createroom', 'doorbell', 'ktvmanage', 'fish', 'balance', 'givefish', 'gamble', 'profile', 'waddle', 'counting', 'qotd', 'publishad', 'boop', 'mail', 'inbox']) { const command = require(`./commands/${file}`); client.commands.set(command.data.name, command); }
...
function scheduleQotd() { const next = new Date(); next.setUTCHours(24, 0, 0, 0); setTimeout(async function run() { for (const guild of client.guilds.cache.values()) await qotd.post(client, guild.id, true).catch((error) => logger.error(`QOTD post failed for ${guild.id}`, error)); scheduleQotd(); }, next.getTime() - Date.now()); }
client.once('clientReady', async () => { client.user.setPresence({ status: 'idle', activities: [{ name: "PikaPeng's Igloo", type: 3 }] }); const typingChannelId = config.typingChannelId(); if (typingChannelId) { const refreshTyping = async () => { const channel = await client.channels.fetch(typingChannelId).catch(() => null); if (channel?.isTextBased()) await channel.sendTyping().catch((error) => logger.error('Typing indicator failed', error)); }; await refreshTyping(); setInterval(refreshTyping, 8000); } doorbell.invalidateAll(); logger.info(`PengBot online as ${client.user.tag}`); try { await cleanupStartup(client); await cleanCommandsOnlyChannel(client); await syncGuildCommands(client.commands); } catch (error) { logger.error('Startup synchronization failed', error); } });
...
client.once('clientReady', scheduleQotd);
...
client.on('interactionCreate', async (i) => { try { if (i.isChatInputCommand()) { if (i.channelId === config.commandsOnlyChannelId() && i.commandName !== 'bump') return i.reply({ content: `${config.pengEmoji()} Only \`/bump\` can be used in this channel.`, ephemeral: true }); const staffRoles = config.staffRoleIds(); if (i.commandName === 'announcements' && !i.member.permissions.has(PermissionFlagsBits.Administrator) && !staffRoles.some((id) => i.member.roles.cache.has(id))) return i.reply({ content: `${config.pengEmoji()} Staff access is required for announcements.`, ephemeral: true }); await client.commands.get(i.commandName)?.execute(i); const reply = await i.fetchReply().catch(() => null); const link = logger.interactionLink(i, reply); logger.info(`${i.user.tag} used /${i.commandName}${link ? ` · ${link}` : ''}`); } else if (i.isButton()) await handleButton(i); else if (i.isModalSubmit()) await handleModal(i); } catch (e) { logger.error('Interaction failed', e); if (!i.replied && !i.deferred) await i.reply({ content: `${config.pengEmoji()} Something went wrong. Please try again later.`, ephemeral: true }).catch(() => {}); } });
