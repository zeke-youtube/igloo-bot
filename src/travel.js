...
async function choose(client, userId, currentGuildId, joinedGuildIds = []) { const choices = (await list(false)).filter((d) => d.guildId !== currentGuildId && !joinedGuildIds.includes(d.guildId)); return choices[Math.floor(Math.random() * choices.length)] || null; }
